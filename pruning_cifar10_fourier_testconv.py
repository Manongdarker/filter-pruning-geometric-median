...
parser.add_argument('--pretrain_path', default='./baseline/resnet20_cifar10_checkpoint.pth.tar', type=str, help='..path of pre-trained model')
...
class Mask:
    ...
    def get_filter_similar(self, weight_torch, compress_rate, distance_rate, length, index_i,max_i, dist_type="l2"):
        codebook = np.ones(length)
        if len(weight_torch.size()) == 4:
            filter_pruned_num = int(weight_torch.size()[0] * (1 - compress_rate))
            similar_pruned_num = int(weight_torch.size()[0] * distance_rate)

            if self.index == index_i:
                filter_pruned_num = 0
                similar_pruned_num = 1
            else:
                filter_pruned_num = 0
    ...
    def init_mask(self, rate_norm_per_layer, rate_dist_per_layer, dist_type ,  index , i , epoch_time = 0 ,):
        self.init_rate(rate_norm_per_layer, rate_dist_per_layer)
        self.index = index
        for index_i, item in enumerate(self.model.parameters()):
            if index_i in self.mask_index:
                # mask for norm criterion
                self.mat[index_i] = self.get_filter_codebook(item.data, self.compress_rate[index_i],
                                                           self.model_length[index])
                self.mat[index_i] = self.convert2tensor(self.mat[index_i])
                if args.use_cuda:
                    self.mat[index_i] = self.mat[index_i].cuda()
                ...
                self.similar_matrix[index_i] = self.get_filter_similar(item.data, self.compress_rate[index_i],
                                                                     self.distance_rate[index_i] ,
                                                                     self.model_length[index_i],index_i,i, dist_type=dist_type)
                self.similar_matrix[index_i] = self.convert2tensor(self.similar_matrix[index_i])
                if args.use_cuda:
                    self.similar_matrix[index_i] = self.similar_matrix[index_i].cuda()
        # print("mask Ready")
